...
import sys
import threading
from common.IE_Util import LOGS_DIR

if sys.version_info.major != 3:
    print("\033[33mPlease use python 3.xxx!\033[0m")
    exit(1)

from common.IE_Util import HDDL_DEMO, REID_DEMO, HDDL_INSTALL_DIR, MODEFILEPATH, \
    IMAGPATH, NETWORKPATH, MO_OUTPUT
from common.IE_Util import MO_PRIVATE_PUBLIC_CNN


if os.name == "nt":
    hddldaemon_exe = os.path.join(r'"C:\\Program Files (x86)\\IntelSWTools\\openvino\\inference_engine\\external\\hddl\\bin', 'hddldaemon.exe"')
    hddldaemon_exe 
...
class DL_MO_test(unittest.TestCase):
    ...
    def test_reid_emb_dis(self):
        test_c = MO_PRIVATE_PUBLIC_CNN()
        cmd=test_c.do_inference_cmd("reidembedding");
        self.assertEqual(MO_PRIVATE_PUBLIC_CNN.run_cmd(cmd),0);
